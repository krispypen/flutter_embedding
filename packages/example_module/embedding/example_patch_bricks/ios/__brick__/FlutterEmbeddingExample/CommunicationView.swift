...
class ExampleHandoversToHostService: HandoversToHostService.SimpleServiceProtocol {
    ...
    func getIncrement(
        request: GetIncrementRequest,
        context: FlutterEmbeddingGRPCCore.ServerContext
    ) async throws -> GetIncrementResponse {
        var response = GetIncrementResponse()
        response.increment = Int32(increment)
        return response
    }
    ...
    func exit(
        ...
        print("Flutter app requested exit with counter: \(counter)")
        ...
        await MainActor.run {
            ...
            alert.addAction(UIAlertAction(title: "OK", style: .default) { _ in
                self.contentViewController?.handleFlutterExit()
            })
...
class CommunicationView: UIViewController {
    ...
    private let incrementMenu = UIButton(type: .system)
    ...
    private let increments = ["1", "2", "3", "4", "5"]
    ...
    private func setupControls() {
        ...
        setupLabelAndMenu(label: incrementLabel, labelText: "Select Increment:", 
                         menu: incrementMenu, options: increments, 
                         currentValue: String(currentIncrement),
                         onChange: { [weak self] value in 
                             if let intValue = Int(value) {
                                 self?.currentIncrement = intValue
                                 self?.handoversToHostService?.increment = intValue
                                 print("Increment changed to: \(intValue)")
                             }
                         })
        ...
        [environmentLabel, environmentMenu, incrementLabel, incrementMenu, themeModeLabel, themeModeMenu, updateThemeButton,
